...
{
    static NSMutableArray *sAlertView = nil;
    if (nil == sAlertView)
    {
        sAlertView = [NSMutableArray array];
    }
    return sAlertView;
}
...
{
    for (UIAlertController *alertController in [self allAlertViewArray])
    {
        [alertController dismissViewControllerAnimated:NO completion:nil];
    }
    
    [[self allAlertViewArray] removeAllObjects];
}
...
- (UILabel *)contentLabel {
    if ([self alertControllerAvailable]) {
        NSArray *views = [self viewArray:_alertController.view];
        if (views.count > 1 && [views[1] isKindOfClass:[UILabel class]]) {
            return views[1];
        }
    }
...
- (void)showWithSender:(id)sender arrowDirection:(UIPopoverArrowDirection)arrowDirection controller:(UIViewController *)controller animated:(BOOL)animated completion:(void (^)(void))completion {
    if ([self alertControllerAvailable]) {
        ...
        if (self.preferredStyle == PSTAlertControllerStyleAlert) {
            ...
            if (!controller) {
                DDLogWarn(@"Can't show alert because there is no root view controller.");
                return;
            }
        }
        ...
        alertController.viewDidDisappearBlock = ^{
            [[PSTAlertController allAlertViewArray] removeObject:weakAlertController];
        ...
        [[PSTAlertController allAlertViewArray] addObject:alertController];
